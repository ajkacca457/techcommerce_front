       ...
       {products && products.map((item)=>{
        return(
            <div className='grid grid-cols-3 gap-x-[10px] border-[1px] border-slate-300 my-8 p-4 items-center group' key={item.id}>
              <img src={item.Image[0].url} className="w-[150px] max-h-[100px] col-span-full md:col-span-1 justify-self-center transition duration-300 group-hover:scale-125"/>
              <div className='text-center col-span-full my-6 md:my-0 md:col-span-1 text-sm'>
                <p>{item.Name}</p>
                <p className='my-2'>{item.Company}</p>
                <p className='font-semibold'>{item.Price}€</p>
              </div>
              <div className='col-span-full md:col-span-1 justify-self-center md:justify-self-end'>
